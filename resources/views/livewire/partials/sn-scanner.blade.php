{{-- resources/views/livewire/partials/sn-scanner.blade.php --}}
{{--
    Dipakai di dalam Livewire component manapun.
    Wajib: property Livewire `public $scannedSerialNumber = null;`
    dan method `public function onSerialScanned($value) {}` (opsional).

    Usage di parent blade:
...
<div
    x-data="{
        ...
        async startScan() {
            ...
            if (!videoEl) { this.isScanning = false; this.isLoading = false; return; }
            ...
            try {
                ...
                this.isLoading = false;
                this.isScanning = false;
                alert('Gagal membuka kamera: ' + (e?.message ?? 'Unknown error') + '. Pastikan izin kamera sudah diberikan.');
            }
        },
        ...
        get zoomPercent() {
            return Math.round(Math.max(0, Math.min(100, ((this.zoomLevel - this.zoomMin) / (this.zoomMax - this.zoomMin)) * 100)));
        },
        ...
        onDecoded(text) {
            ...
            // kirim hasil scan ke Livewire component parent
        ...
        stopScan() {
            ...
            if (this.$refs.video) this.$refs.video.style.transform = '';
...
>
    <button
        ...
        class="flex flex-col items-center justify-center w-full gap-2 px-4 transition-all border-2 border-dashed cursor-pointer py-7 border-main-primary/50 rounded-2xl bg-main-primary/5 hover:bg-main-primary/10 dark:bg-white/5 dark:hover:bg-white/10 group"
    >
        <div class="p-3 transition-transform bg-white rounded-full shadow dark:bg-white/10 text-main-primary group-hover:scale-110">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-9 h-9" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    M3.75 14.625c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5a1.125 1.125 0 01-1.125-1.125v-4.5z
                    M13.5 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0113.5 9.375v-4.5z"/>
            </svg>
        </div>
        <div class="text-center">
            <p class="text-sm font-bold text-black dark:text-white">Buka Scanner Barcode / QR</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Tap untuk aktifkan kamera & scan SN produk</p>
        </div>
    </button>

    <div x-cloak x-show="isScanning" class="relative w-full overflow-hidden bg-black border border-gray-700 shadow-xl rounded-2xl" style="height: 380px;">
        <video x-ref="video" class="absolute inset-0 w-full h-full" muted playsinline></video>

        <div x-show="!isLoading" class="absolute inset-0 pointer-events-none" style="background:
            ...
            linear-gradient(to left,   rgba(0,0,0,.52) 6%,  transparent 6%);"></div>

        <div x-show="!isLoading && !scanSuccess" class="sc-sweep absolute left-[6%] right-[6%] pointer-events-none" style="height: 1px;
            background: linear-gradient(90deg,transparent,#4ade80 25%,#22c55e,#4ade80 75%,transparent);
            box-shadow: 0 0 8px 2px rgba(74,222,128,.45);"></div>
        ...
        <template x-if="!isLoading && !scanSuccess">
            <div class="absolute pointer-events-none sc-corner" style="inset: 9% 6% 19% 6%;">
                ...
                <div class="absolute inset-x-0 flex justify-center -bottom-6">
                    <span class="text-[11px] font-medium text-green-300/70 tracking-wide select-none">Posisikan barcode dalam kotak</span>
                </div>
            </div>
        </template>

        <div x-show="!isLoading && !scanSuccess" class="absolute bottom-0 left-0 right-0 z-20 px-4 pt-8 pb-3" style="background: linear-gradient(to top, rgba(0,0,0,.8) 0%, transparent 100%);">
            <div class="flex items-center gap-3">
                <button type="button" x-on:click="zoomOut()" class="flex items-center justify-center flex-shrink-0 text-white transition-colors rounded-full w-7 h-7 bg-white/15 hover:bg-white/30">
                ...
                <input type="range" class="flex-1 zoom-slider" :min="zoomMin" :max="zoomMax" :step="zoomStep" :value="zoomLevel"
                ...
                <button type="button" x-on:click="zoomIn()" class="flex items-center justify-center flex-shrink-0 text-white transition-colors rounded-full w-7 h-7 bg-white/15 hover:bg-white/30">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
                        <path fill-rule="evenodd" d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z" clip-rule="evenodd"/>
                ...
                <span class="flex-shrink-0 min-w-[30px] text-right text-[11px] font-mono font-medium text-green-300" x-text="zoomLabel"></span>
                <button type="button" x-on:click="stopScan()" class="flex-shrink-0 flex items-center gap-1 px-2.5 py-1 ml-1 rounded-full bg-red-600/80 hover:bg-red-500 text-white text-[11px] font-semibold transition-colors">
        ...
        <div x-show="isLoading" class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/85">
        ...
        <div x-show="scanSuccess" class="absolute inset-0 z-20 flex flex-col items-center justify-center" style="background: rgba(0,0,0,.72);">
