<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
        ...
        @foreach ($documents as $document)
            ...
            <a href="{{ $url }}" 
               ...
               class="relative flex items-center p-3 space-x-3 transition-all duration-200 border shadow-sm group rounded-xl bg-main-light border-border-light hover:bg-secondary-light hover:border-main-primary/30 dark:bg-main-dark dark:border-border-dark dark:hover:bg-secondary-dark dark:hover:border-main-primary/30">
                ...
                {{-- Download/Arrow Icon --}}
                <div>
                    <x-heroicon-m-arrow-down-tray class="w-4 h-4 text-gray-400 transition-transform group-hover:text-main-primary group-hover:-translate-y-0.5" />
                </div>
            </a>
        @endforeach
    </div>
    ...
    @if(empty($documents))
        <div class="flex flex-col items-center justify-center p-6 text-center border border-dashed rounded-xl border-border-light dark:border-border-dark">
            <x-heroicon-o-document class="w-10 h-10 mb-2 text-gray-400" />
