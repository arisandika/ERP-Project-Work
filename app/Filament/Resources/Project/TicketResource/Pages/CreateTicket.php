    ...
    protected function fillForm(): void
    {
        ...
        if ($copyFromId) {
            ...
            if ($ticket) {
                ...
                $data['assignees'] = $ticket->assignees()->pluck('nx_employees.id')->toArray();
                $this->form->fill($data);
        ...
        if (!empty($data['assignees']) && !empty($data['project_id'])) {
            ...
            if ($project) {
                ...
                foreach ($data['assignees'] as $employeeId) {
                    $isMember = $project->members()->where('nx_employees.id', $employeeId)->exists();
                    
                    if ($isMember) {
                        $validAssignees[] = $employeeId;
                    } else {
                        $invalidAssignees[] = $employeeId;
                    }
                }
